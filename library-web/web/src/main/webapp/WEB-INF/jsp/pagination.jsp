	...
	<div id="page" class="page">
		...
		<c:if test="${requestScope.currentPage != 1}">
			<form action="${pageContext.request.contextPath}/books" method="post">
				<input type="hidden" value="${requestScope.currentPage - 1}"
					name="page">
				...
				<button class="simpleButoon" type="submit">
					<fmt:message key="page.pagination.prev" />
				</button>
			</form>
		</c:if>
		...
		<c:choose>
			<c:when
				test="${requestScope.currentPage > (1+2) and requestScope.currentPage < (requestScope.numOfPages - 2)}">
				...
				<c:forEach begin="${requestScope.currentPage - 2}"
					end="${requestScope.currentPage + 2}" var="i">
					<form action="${pageContext.request.contextPath}/books"
						method="post">
			...
			<c:when test="${requestScope.currentPage > (1 + 2)}">
				...
				<c:forEach begin="${requestScope.currentPage - 2}"
					end="${requestScope.numOfPages}" var="i">
					<form action="${pageContext.request.contextPath}/books"
						method="post">
						<input type="hidden" value="${i}" name="page">
						<c:if test="${not empty requestScope.name}">
			...
			<c:when
				test="${requestScope.currentPage < (requestScope.numOfPages - 2)}">
		...
		<c:if test="${requestScope.currentPage != requestScope.numOfPages}">
			<form action="${pageContext.request.contextPath}/books" method="post">
				...
				<button class="simpleButoon" type="submit">
					<fmt:message key="page.pagination.next" />
				</button>
				<br>
			</form>
		</c:if>
